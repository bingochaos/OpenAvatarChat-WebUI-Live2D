...
export class PluginRegistry {
  ...
  async loadPlugin(folderPath: string): Promise<boolean> {
    ...
    if (this.plugins.has(manifest.id)) {
      this.logger.warn('Plugin already loaded', { id: manifest.id })
      return false
    }
  ...
  async initAll(configs?: Record<string, Record<string, any>>): Promise<void> {
    const tasks = [...this.plugins.entries()].map(async ([id, entry]) => {
      ...
      try {
        ...
        // Remove its tools so they don't get routed
  ...
  /**
   * Unload a plugin, then reload it from disk.
   * Useful for development or updating plugin code at runtime.
   */
