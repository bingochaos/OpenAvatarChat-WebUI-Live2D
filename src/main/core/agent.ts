...
export class Agent {
  ...
  constructor(
    ...
    this.hasApiKey = Boolean(apiKey && apiKey.length > 0)
    this.client = this.hasApiKey ? new Anthropic({ apiKey }) : null
    this.opts = { ...DEFAULTS, ...opts }
    this.logger = new Logger('agent')
    if (!this.hasApiKey) {
      this.logger.warn('ANTHROPIC_API_KEY missing — agent will refuse to run')
    }
  }
  ...
  async *run(userMessage: string): AsyncGenerator<AgentEvent> {
    if (!this.client) {
      yield {
        type: 'error',
        message: '未配置 ANTHROPIC_API_KEY，请在 .env 中补充后重启应用。',
        recoverable: false,
      }
      return
    }
      ...
      for (; round < this.opts.maxRounds; round++) {
        ...
        for (const block of toolBlocks) {
          if (signal.aborted) {
            yield {
              type: 'error',
              message: 'Aborted during tool execution',
              recoverable: true,
            }
            return
          }
  ...
  /** Whether the agent has a usable API key. */
  get ready(): boolean {
